...
- enemigos, bosses, UFOs, power-ups y proyectiles
...
- bestiario
- campaña y sectores
- colección de skins, naves e insignias
- desafío activo
...
2. `loop()` coordina `update()` y `draw()` con `requestAnimationFrame`.
3. `update()` resuelve input, movimiento, colisiones, progresión, meta y cambios de estado.
4. `draw()` renderiza el estado completo sobre el `canvas`.
5. `setOverlayMode()` recompone la UI contextual para inicio, pausa o cierre.
